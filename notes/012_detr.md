...
该节大概讲了三个部分：一是先讲了一下集合预测这个问题，以及之前大家都是用什么方法去解决集合预测问题；二是介绍了一下transformer以及parallel decoding，就是如何不像之前的transformer decoder一样去做那种自回归的预测，而是一次性的将预测结果返回；三是介绍了目标检测之前的相关工作。GT
该节大概讲了三个部分：一是先讲了一下集合预测这个问题，以及之前大家都是用什么方法去解决集合预测问题；二是介绍了一下transformer以及parallel decoding，就是如何不像之前的transformer decoder一样去做那种自回归的预测，而是一次性的将预测结果返回；三是介绍了目标检测之前的相关工作。
